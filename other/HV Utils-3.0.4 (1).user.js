...
// @name           HV Utils
// @namespace      HVUT
// @description    A comprehensive out-of-battle script for Hentaiverse，异世界更新过渡随便用用版本
...
var settings = {
  ...
  topMenuLinks: ['Character', 'Equipment', 'Item Inventory', 'Equip Inventory', 'Equipment Shop', 'The Armory', 'Item Shop', 'The Market', 'Monster Lab', 'The Shrine', 'MoogleMail', 'The Arena', 'The Tower', 'Ring of Blood', 'GrindFest', 'Item World'],
  ...
  equipNameCode: [
    ...
    'Leather && (Protection || Fleet) : type=bold, suffix=bold',
    'Kevlar && (Protection || Fleet) : type=bold, suffix=bold',
    'Drakehide && (Protection || Fleet) : type=bold, suffix=bold',
...
var $equip = {
  ...
  index: {
    ...
    type: {
      ...
      'Phase': 1, 'Cotton': 2, 'Gossamer': 3, 'Silk': 4, 'Ironsilk': 5,
      'Shade': 1, 'Leather': 2, 'Kevlar': 3, 'Drakehide': 4,
      'Power': 1, 'Plate': 2, 'Shield': 3, 'Chainmail': 4, 'Chain': 5, 'Reactive': 6,
    },
  ...
  reg: {
    name: (() => {
      ...
      const acloth = 'Cotton|Phase|Gossamer|Silk|Ironsilk';  // Added "Ironsilk" for Isekai
      const alight = 'Leather|Shade|Kevlar|Drakehide';
      const aheavy = 'Plate|Power|Shield|Chainmail|Chain|Reactive';  // Added "Chain" and "Reactive" for Isekai
  ...
  parse: {
    ...
    div: function (div) {
      ...
      // Try Isekai format first (check for <div class="eqt">)
      // Matches: "Tier X / Y / Z", "Level 123", or "Level Unassigned"
      const isekaiExec = /<div class="eqt">(.+?) &nbsp; &nbsp; (?:Tier (\d+) \/ (\d+) \/ (\d+)|Level (Unassigned|\d+))? ?&nbsp; ?&nbsp; <span>(Tradeable|Soulbound)<\/span><\/div>/.exec(dynjs.d);
      if (isekaiExec) {
        // Isekai format - extract condition percentage
        const condMatch = /Condition: (\d+)% /.exec(dynjs.d);
        const condition = condMatch ? parseInt(condMatch[1]) : 100;

        // Parse level: can be "Unassigned" or a number
        const levelValue = isekaiExec[5];
        const isUnassigned = levelValue === 'Unassigned';
        const level = isUnassigned ? 0 : parseInt(levelValue) || 0;

        const eq = {
          info: {
            name: dynjs.t,
            category: isekaiExec[1],
            level: level,
            unassigned: isUnassigned,
            tradeable: isekaiExec[6] === 'Tradeable',
            soulbound: isekaiExec[6] === 'Soulbound',
            cdt: condition / 100,
            condition: condition,
            durability: 100,
            tier: parseInt(isekaiExec[2]) || 0,
            pxp1: parseInt(isekaiExec[3]) || 0,
            pxp2: parseInt(isekaiExec[4]) || 0,
  ...
  list: function (node, sort = true, parent = node) {
    ...
    const equiplist = Array.from($qsa('div[onmouseover*="equips.set"]', node)).map((div) => {
      ...
      // Check if parsing failed (error object without node property)
      if (!eq.node) {
        return null;
      }
      ...
      return eq;
    }).filter(eq => eq !== null);  // Filter out failed parses
...
_top.menu = {
  ...
  'Equip Inventory': { s: 'Character', ss: 'in', isekai: false },
  ...
  'Equipment Shop': { s: 'Bazaar', ss: 'es', isekai: false },
  'The Armory': { s: 'Bazaar', ss: 'am', isekai: true },
  'Modify': { s: 'Bazaar', ss: 'am', screen: 'modify', isekai: true },
  'Repair': { s: 'Bazaar', ss: 'am', screen: 'repair', isekai: true },
  'Organize': { s: 'Bazaar', ss: 'am', screen: 'organize', isekai: true },
  'Soulbindisk': { s: 'Bazaar', ss: 'am', screen: 'soulbind', isekai: true },
  'Purchase': { s: 'Bazaar', ss: 'am', screen: 'purchase', isekai: true },
  'Sell': { s: 'Bazaar', ss: 'am', screen: 'sell', isekai: true },
  'Salvageisk': { s: 'Bazaar', ss: 'am', screen: 'salvage', isekai: true },
  ...
  'Repair': { s: 'Forge', ss: 're', isekai: false },
  'Upgrade': { s: 'Forge', ss: 'up', isekai: false },
  'Enchant': { s: 'Forge', ss: 'en', isekai: false },
  'Salvage': { s: 'Forge', ss: 'sa', isekai: false },
  'Reforge': { s: 'Forge', ss: 'fo', isekai: false },
  'Soulfuse': { s: 'Forge', ss: 'fu', isekai: false },
};
...
_top.create = function () {
  ...
  Object.entries(_top.menu).forEach(([k, m]) => {
    ...
    const url = m.href || `?s=${m.s}&ss=${m.ss}${m.screen ? '&screen=' + m.screen : ''}`;
    ul[m.s].appendChild($element('li')).appendChild($element('a', null, { textContent: k, href: url }));
  });
  /* monsterbation */
  if ($id('mbsettings')) {
...
var $persona = {
  ...
  check_e: function (doc = document) {
    const json = $persona.json;
    const pidx = json.pidx;
    if (!json[pidx]) {
      json[pidx] = {};
    }
    const eqSlots = $id('eqsl', doc);
    let currentImg = eqSlots && $qs('#eqsl img[src$="_on.png"]', doc);
    if (!currentImg) {
      currentImg = $qs('img[src$="_on.png"]', doc);
    }

    let eidx;
    let elen;

    if (currentImg && eqSlots) {
      eidx = parseInt(currentImg.src.match(/set(\d+)_on\.png/)?.[1] || currentImg.src.slice(-8, -7));
      elen = eqSlots.childElementCount;
    } else {
      // Equip slot pages (equip_slot parameter) do not render #eqsl
      const hiddenSlot = $qs('input[name="equip_slot"]', doc);
      if (!hiddenSlot) {
        console.warn('[HV Utils] Unable to detect current equip slot');
        return;
      }
      eidx = parseInt(hiddenSlot.value, 10) || json.eidx || 1;
      elen = json.elen || eidx;
    }
  ...
  load_dynjs: async function (doc) {
    async function finalize() {
      ...
      if (_query.s === 'Battle') {
        $battle?.create();
      } else if (!_isekai && ['eq', 'ab', 'it', 'se'].includes(_query.ss)) {
        location.href = location.href;
      }
    }

    if (_isekai) {
      const script = $qs('script[src*="/dynjs/"][src*="/equip-"]', doc);
      if (!script) {
        console.warn('[HV Utils] Isekai equip script not found on Character Equip page');
        await finalize();
        return;
      }
      try {
        const jsText = await $ajax.fetch(script.src + '?t=' + Date.now());
        const equipMatch = /(?:var|let|const)\s+dynjs_equip\s*=\s*(\{[\s\S]*?\});/s.exec(jsText);
        if (equipMatch) {
          Object.assign($equip.dynjs_equip, JSON.parse(equipMatch[1]));
        }
        const storeMatch = /(?:var|let|const)\s+dynjs_eqstore\s*=\s*(\{[\s\S]*?\});/s.exec(jsText);
        if (storeMatch) {
          Object.assign($equip.dynjs_eqstore, JSON.parse(storeMatch[1]));
        }
      } catch (err) {
        console.warn('[HV Utils] Isekai equip dynjs parse failed:', err);
      }
      await finalize();
      return;
    }
    ...
    if (!src) {
      console.warn('[HV Utils] Cannot find dynjs script tag');
      return;
    }
  ...
  save_equipset: function (doc) {
    ...
    const equipset = $qsa('.eqb', doc).map((d) => {
      ...
      const eqDiv = d.children[1];
      const eq = $equip.parse.div(eqDiv);
      const label = eqDiv.textContent.trim();
      const hasHover = /equips\.set/.test(eqDiv.getAttribute('onmouseover') || '');
      if (eq.info) {
        const { category, name, customname, eid, key } = eq.info;
        return { slot, category, name, customname, eid, key };
      } else {
        const isEmptySlot = !hasHover || !label || /^empty$/i.test(label);
        if (!isEmptySlot) {
          hasMissing = true;
        }
        return { slot };
    ...
    if (hasMissing) {
      console.warn('[HV Utils] Incomplete equipset data detected, keeping previous cache');
      return false;
    }
  ...
  parse_stats_pane: function (doc) {
    ...
    const spn_element = $qs('.spn', doc);
    const fighting_style = spn_element && /(Unarmed|One-Handed|Two-Handed|Dualwield|Niten Ichiryu|Staff)/.test(spn_element.textContent) && RegExp.$1;
  ...
  _bottom.show_equip = async function () {
    ...
    try {
      if (!_isekai) {
        ...
        if (!exec) {
          _bottom.node.equip.textContent = 'Equip Slots: N/A';
          return;
        }
        ...
        // Isekai: parse capacities from Armory Organize page
        const html = await $ajax.fetch('?s=Bazaar&ss=am&screen=organize&filter=new');
        const inv = /Inventory Capacity:<\/td><td>(\d+)<\/td><td>\/<\/td><td>(\d+)/.exec(html);
        if (!inv) {
          _bottom.node.equip.textContent = 'Equip Slots: N/A';
          return;
        }
        const invUsed = parseInt(inv[1]);
        const invCap = parseInt(inv[2]);
        const free = invCap - invUsed;
        _bottom.node.equip.textContent = `Inventory Capacity: ${invUsed} / ${invCap}`;
        if (free < invCap / 10) {
          _bottom.node.equip.classList.add('hvut-bottom-warn');
        } else if (free < invCap / 2) {
          _bottom.node.equip.style.color = '#c00';
        }
      }
    } catch (e) {
...
if (settings.equipment && !_isekai && _query.s === 'Character' && _query.ss === 'eq') {
...
} else if (settings.equipment && _isekai && _query.s === 'Character' && _query.ss === 'eq') {
  // Isekai Character Equipment - Simple coloring only
  GM_addStyle(/*css*/`
    #eqsb .eqb > div:last-child { position: relative; }
    #eqsb .hvut-eq-Superior { background-color: #ccc; }
    #eqsb .hvut-eq-Exquisite { background-color: #ce9; }
    #eqsb .hvut-eq-Magnificent { background-color: #bdf; }
    #eqsb .hvut-eq-Legendary { background-color: #fd8; }
    #eqsb .hvut-eq-Peerless { background-color: #fbb; }
    #equiplist tr.hvut-eq-Superior { background-color: #ccc; }
    #equiplist tr.hvut-eq-Exquisite { background-color: #ce9; }
    #equiplist tr.hvut-eq-Magnificent { background-color: #bdf; }
    #equiplist tr.hvut-eq-Legendary { background-color: #fd8; }
    #equiplist tr.hvut-eq-Peerless { background-color: #fbb; }
  `);

  // Refresh current equipset/cache like main world
  $persona.check_e(document);
  $persona.set_button();
  $persona.load_dynjs(document);

  // Apply coloring to equipped items
  $qsa('#eqsb .eqb > div[onmouseover*="equips.set"]').forEach((div) => {
    const eid = /equips\.set\((\d+),/.test(div.getAttribute('onmouseover')) && RegExp.$1;
    if (!eid) return;

    const dynjs = $equip.dynjs_equip[eid];
    if (!dynjs) return;

    const eq = { info: { name: dynjs.t } };
    $equip.parse.name(eq.info.name, eq);
  ...
  // Apply coloring to equipment switching page (#equiplist)
  const applyEquipListColoring = function () {
    $qsa('#equiplist tr[onmouseover*="hover_equip"]:not([data-colored])').forEach((tr) => {
      const onmouseover = tr.getAttribute('onmouseover');
      const eid = /hover_equip\((\d+)\)/.test(onmouseover) && RegExp.$1;
      if (!eid) return;
      ...
      if (eq.info.quality) {
        tr.classList.add('hvut-eq-' + eq.info.quality);
      }
      tr.dataset.colored = '1';  // Mark as processed
    });
  };

  // Apply coloring on page load
  applyEquipListColoring();

  // Watch for dynamic changes (when switching equipment slots)
  const equipList = $id('equiplist');
  if (equipList) {
    (new MutationObserver(() => {
      applyEquipListColoring();
    })).observe(equipList, { childList: true, subtree: true });
  }
}
...
if (settings.equipInventory && _query.s === 'Character' && _query.ss === 'in') {
  ...
  _in.edit_equipcode = function () {
    const comment = `
      // {$name}       装备名称
      // {$namecode}   带颜色/粗体的装备名称
      // {$url}        装备链接地址
      // {$eid}        装备ID
      // {$_eid}       带透明下划线对齐的装备ID（用于论坛排版对齐）
      // {$level}      装备等级
      // {$pab}        装备属性值（potency/attack/block）
      // {$tier}       效能等级（IW等级）
      // {$price}      价格输入框的值
      // {$note}       备注输入框的值
      //               如果包含'$featured;'，装备代码会添加到'Featured'区域
      //               如果包含'$new;'，装备代码会添加到'Newly Added'区域
      // {$condition ? text_if_true : text_if_false}
      //               条件表达式：如果$condition有效，输出'text_if_true'，否则输出'text_if_false'
      //               例如 {$level ? Lv.$level : Soulbound}
      //               如果$level是500，输出'Lv.500'
      //               如果没有$level信息，输出'Soulbound'
    `.replace(/^\s+/gm, '').trim();
  ...
  _in.edit_namecode = function () {
    const comment = `
      // 基础匹配 : 属性=值, 属性=值, ...
      // 基础匹配 : 属性=值, 属性=值, ... ; 子匹配 : 属性=值, 属性=值, ... ; 子匹配 : 属性=值, 属性=值, ...
      // 可用属性 : name(名称), quality(品质), prefix(前缀), type(类型), slot(槽位), suffix(后缀)
      // 可用值 : bold(粗体), rainbow(彩虹色), 或任何颜色如'red', '#f00'
    `.replace(/^\s+/gm, '').trim();
...
//* [8] Bazaar - Equipment Shop (ES in normal, AM in Isekai)
// Isekai AM is always enabled, Persistent ES depends on settings
if (_query.s === 'Bazaar' && (settings.equipmentShop && _query.ss === 'es' || _isekai && _query.ss === 'am')) {
  _es.ss = _query.ss; // Store current ss parameter (es or am)
  ...
  // Isekai AM uses equipform, Persistent ES uses shopform
  const shopform = $id('shopform');
  _es.storetoken = shopform?.elements?.storetoken?.value || '';
  _es.has_storetoken = !!_es.storetoken;
  _es.item_pane = [];
  _es.shop_pane = [];
  _es.node = {};
  ...
  _es.sell = async function (eq, ask = true) {
    ...
    const html = await $ajax.fetch(`?s=Bazaar&ss=${_es.ss}`, `storetoken=${_es.storetoken}&select_group=item_pane&select_eids=${eq.info.eid}`);
  ...
  _es.shop_buy = async function (eq, ask = true) {
    ...
    const html = await $ajax.fetch(`?s=Bazaar&ss=${_es.ss}`, `storetoken=${_es.storetoken}&select_group=shop_pane&select_eids=${eq.info.eid}`);
    _es.update_credits(html);
    eq.node.wrapper.remove();
  };
  ...
  _es.sell_all = async function () {
    ...
    const html = await $ajax.fetch(`?s=Bazaar&ss=${_es.ss}`, `storetoken=${_es.storetoken}&select_group=item_pane&select_eids=${select_eids}`);
    _es.update_credits(html);
  ...
  _es.load_list = async function (filter) {
    ...
    const html = await $ajax.fetch(`?s=Bazaar&ss=${_es.ss}&filter=` + filter);
    const doc = $doc(html);
    Object.assign($equip.dynjs_eqstore, JSON.parse(/var dynjs_eqstore = (\{.*\});/.test(html) && RegExp.$1));
  ...
  // Persistent Equipment Shop UI
  if (!_isekai && _query.ss === 'es' && $id('eqshop_outer')) {
    GM_addStyle(/*css*/`
    ...
    if (settings.equipmentShopIntegration) {
      const filterbar = $id('filterbar');
      $element('a', [filterbar, 'afterbegin'], { href: `?s=Bazaar&ss=${_es.ss}`, innerHTML: '<div>All</div>' });
      if (_query.filter) {
        filterbar.children[0].children[0].classList.add('cfb');
      } else {
        filterbar.children[0].children[0].classList.add('cfbs');
  ...
  //* [8.5] Isekai Armory - Equip Code Export
  if (_isekai && _query.ss === 'am' && $id('armory_outer')) {
    _am = _am || {};
    _am.equiplist = [];
    _am.equipdata = getValue('in_equipdata', {});
    _am.equipcode = getValue('in_equipcode', settings.equipCode);
    _am.namecode = getValue('in_namecode', settings.equipNameCode);
    _am.node = {};
    _am.defaultBazaarFilter = [
      'Peerless',
      'Legendary',
      'Magnificent',
      'Exquisite',
    ...
    // Quality name mapping for Chinese translation compatibility
    _am.qualityMap = {
      '☯无双☯': 'Peerless',
      '✪传奇✪': 'Legendary',
      '☆史诗☆': 'Magnificent',
    ...
    // Convert Chinese equipment name back to English for filtering
    _am.normalizeEquipName = function (name) {
      let normalized = name;
      for (const [cn, en] of Object.entries(_am.qualityMap)) {
        normalized = normalized.replace(cn, en);
      }
      return normalized;
    };

    // Event handler for Base value extraction buttons
    _am.click = function (e) {
      const target = e.target.closest('[data-action]');
      if (!target) { return; }
      const { action, eid } = target.dataset;
      const eq = eid && _am.equiplist.find((eq) => eq.info.eid == eid);
      if (action === 'lpr') {
        _am.get_base(eq);
      }
    };

    // Parse equipment list from AM page (table rows)
    _am.parse_list = function () {
      _am.equiplist = [];
      const rows = $qsa('#equiplist table tr[onmouseover*="hover_equip"]');
      ...
      rows.forEach((tr) => {
        ...
        // For pages without dynjs data, use label text to create basic equipment object
        if (!dynjs) {
          const label = tr.querySelector('label');
          if (!label) { return; }

          // Get raw name first (keep quality symbols for normalization)
          const rawName = label.textContent.trim();
          const detectedQuality = (() => {
            for (const [cn, en] of Object.entries(_am.qualityMap)) {
              if (rawName.includes(cn)) {
                return en;
              }
            }
            return null;
          })();
          // Normalize Chinese quality names to English FIRST (before removing emojis)
          const normalized = _am.normalizeEquipName(rawName);
          // Then remove remaining emojis and special characters (but not quality symbols which are now converted)
          const equipName = normalized.replace(/[\u{1F000}-\u{1F9FF}\u{FE00}-\u{FE0F}\u{1F300}-\u{1F5FF}\u{1F600}-\u{1F64F}\u{1F680}-\u{1F6FF}\u{1F900}-\u{1F9FF}⚠️]/gu, '').trim();
          ...
          const eq = {
            ...
            node: {
              tr: tr,
              checkbox: $qs('input[type="checkbox"]', tr),
            },
            json: _am.equipdata[eid] || {},
          };

          $equip.parse.name(eq.info.name, eq);
          if (!eq.info.quality && detectedQuality) {
            eq.info.quality = detectedQuality;
          }
          ...
          // Add custom input fields (only on organize page)
          if (_query.screen === 'organize') {
            const targetCell = tr.cells[1] || tr.cells[0];
            if (targetCell) {
              eq.node.sub = $element('span', [targetCell, 'beforeend'], ['.hvut-am-sub']);
              eq.node.price = $input('text', eq.node.sub, { className: 'hvut-am-price', placeholder: 'price', value: '' });
              eq.node.note = $input('text', eq.node.sub, { className: 'hvut-am-note', placeholder: 'note', value: '' });
              eq.node.lpr = $input(['button', '<'], eq.node.sub, { className: 'hvut-am-lpr', dataset: { action: 'lpr', eid: eq.info.eid } });
            }
          }

          // Filter Purchase page
          if (_query.screen === 'purchase') {
            const bazaarFilter = (getValue('am_bazaar', _am.defaultBazaarFilter) || []).map(f => f.trim()).filter(f => f);
            const visible = $equip.filter(equipName, bazaarFilter);
            if (!visible) {
              tr.style.display = 'none';
            }
          }

          _am.equiplist.push(eq);
          return;
        }

        // Isekai uses different HTML format - support "Level 123", "Level Unassigned", or "Tier X / Y / Z"
        const isekaiMatch = /<div class="eqt">(.+?) &nbsp; &nbsp; (?:Tier (\d+) \/ (\d+) \/ (\d+)|Level (Unassigned|\d+))? ?&nbsp; ?&nbsp; <span>(Tradeable|Soulbound)<\/span><\/div>/.exec(dynjs.d);
        if (!isekaiMatch) { return; }
        ...
        // Parse level: can be "Unassigned" or a number
        const levelValue = isekaiMatch[5];
        const isUnassigned = levelValue === 'Unassigned';
        const level = isUnassigned ? 0 : parseInt(levelValue) || 0;
        ...
        const eq = {
          info: {
            name: dynjs.t,
            category: isekaiMatch[1],
            level: level,
            unassigned: isUnassigned,
            tradeable: isekaiMatch[6] === 'Tradeable',
            soulbound: isekaiMatch[6] === 'Soulbound',
            cdt: condition / 100,
            condition: condition,
            durability: 100,
            tier: parseInt(isekaiMatch[2]) || 0,
            pxp1: parseInt(isekaiMatch[3]) || 0,
            pxp2: parseInt(isekaiMatch[4]) || 0,
            pab: (dynjs.d.match($equip.reg.pab) || []).map((p) => p[0]).join(''),
            eid: eid,
            key: dynjs.k,
          },
        ...
        // Parse equipment name details
        $equip.parse.name(eq.info.name, eq);
        eq.info.pxp = $equip.getpxp(eq);

        // Add quality coloring
        if (eq.info.quality) {
          tr.classList.add('hvut-eq-' + eq.info.quality);
        }
        ...
        // Add custom input fields (only on organize page)
        if (_query.screen === 'organize') {
          // Organize page has 1 column, other pages have 2 columns
          const targetCell = tr.cells[1] || tr.cells[0];
          if (targetCell) {
            eq.node.sub = $element('span', [targetCell, 'beforeend'], ['.hvut-am-sub']);
            eq.node.price = $input('text', eq.node.sub, { className: 'hvut-am-price', placeholder: 'price', value: '' });
    ...
    // Generate WTS code
    _am.wts = function () {
      const equiplist = _am.equiplist.filter((e) => e.node.checkbox.checked);
      const eid_maxlen = Math.max(...equiplist.map((e) => e.info.eid.toString().length));
      const includeHeaders = getValue('am_include_headers', true); // default: include headers
      let code = '';
      let code_new = '';
      let code_featured = '';
      ...
      equiplist.forEach((eq, i, a) => {
        ...
        // Add sequential number for custom prefix (e.g., yan01, yan02, ...)
        eq.data._seq = String(i + 1).padStart(2, '0');
        if (!eq.data.url && eq.info.key) {
          eq.data.url = `${location.origin}${location.pathname}equip/${eq.info.eid}/${eq.info.key}`;
        }
        eq.data.namecode = $equip.namecode(eq);
        eq.data.price = eq.node.price ? eq.node.price.value : '';
        eq.data.note = eq.node.note ? eq.node.note.value : '';
        ...
        const equipcode = _am.equipcode.replace(/\{\$(\w+)(\s*\?(.*?)(?::(.*?))?)?\}/g, (s, k, e, t, f) => {
          const v = (k in eq.data) ? eq.data[k] : (k in eq.info) ? eq.info[k] : '';
          if (!e) {
            return v ?? '';
          } else {
      ...
      popup_text(code.trim() || 'No equipment selected.', 'width: 900px; height: 500px; white-space: pre;', [
        { value: 'Toggle Headers', click: (w, t) => {
          const current = getValue('am_include_headers', true);
          setValue('am_include_headers', !current);
          w.remove();
          _am.wts();
        } },
        { value: 'Equip Code Format', click: () => { _am.edit_equipcode(); } },
        { value: 'Name Code Rule', click: () => { _am.edit_namecode(); } },
      ]);
    };

    _am.edit_equipcode = function () {
      const comment = `
        // {$name}       装备名称
        // {$namecode}   带颜色/粗体的装备名称
        // {$url}        装备链接地址
        // {$level}      装备等级
        // {$soulbound}  如果是灵魂绑定则显示'Soulbound'
        // {$unassigned} 如果是未分配则显示'Unassigned'
        // {$pab}        装备属性值（potency/attack/block）
        // {$tier}       效能等级（IW等级）
        // {$price}      价格输入框的值
        // {$note}       备注输入框的值
        ...
        //               如果包含'$new;'，装备代码会添加到'Newly Added'区域
        // {$_eid}       装备UID（默认）
        // {$_seq}       装备序号（01, 02, 03...）可用于自定义前缀，例如：[yan{$_seq}]
        // {$condition ? text_if_true : text_if_false}
        //               条件表达式：如果$condition有效，输出'text_if_true'，否则输出'text_if_false'
        //               例如 {$level ? Lv.$level : Soulbound}
        //               如果$level是500，输出'Lv.500'
        //               如果没有$level信息，输出'Soulbound'
      `.replace(/^\s+/gm, '').trim();
      popup_text(comment + '\n\n' + _am.equipcode, 'width: 900px; height: 500px; white-space: pre;', [
        { value: 'Save', click: (w, t) => {
          _am.equipcode = (t.value || settings.equipCode).split('\n').filter((e) => !e.startsWith('//')).join('\n').trim();
          setValue('in_equipcode', _am.equipcode);
          w.remove();
        } },
        { value: 'Default', click: (w, t) => {
          t.value = comment + '\n\n' + settings.equipCode;
    ...
    _am.edit_namecode = function () {
      const comment = `
        // 过滤条件 : 属性设置
        // {条件} && {条件} : 匹配所有条件（与）
        // {条件} || {条件} : 匹配任一条件（或）
        // Peerless, Legendary, Magnificent, Exquisite : 品质(quality)
        // Slaughter, Destruction : 后缀(suffix)
        // Ethereal, Hallowed, Demonic : 前缀(prefix)
        // Shortsword, Katalox, Shade : 类型(type)
        // quality, prefix, type, suffix, name : 可修改的属性
        // quality=#f90 或 quality=bold 或 quality=rainbow（rainbow是Peerless默认值）
      `.replace(/^\s+/gm, '').trim();
      popup_text(comment + '\n\n' + _am.namecode.join('\n'), 'width: 900px; height: 500px; white-space: pre;', [
        { value: 'Save', click: (w, t) => {
          _am.namecode = (t.value || settings.equipNameCode.join('\n')).split('\n').filter((e) => !e.startsWith('//')).map((e) => e.trim()).filter((e) => e);
          setValue('in_namecode', _am.namecode);
          w.remove();
        } },
        { value: 'Default', click: (w, t) => {
          t.value = comment + '\n\n' + settings.equipNameCode.join('\n');
    ...
    _am.edit_filter = function () {
      const comment = '// 将指定品质保留在购买页面，然后隐藏其他';
      const current = getValue('am_bazaar', _am.defaultBazaarFilter);
      popup_text(comment + '\n\n' + current.join('\n'), 'width: 600px; height: 500px; white-space: pre;', [
        { value: 'Save Filter', click: (w, t) => {
          const edited = t.value.split('\n').filter((f) => f && !f.startsWith('//'));
          setValue('am_bazaar', edited);
          w.remove();
          location.reload();
        } },
        { value: 'Default', click: (w, t) => {
          t.value = comment + '\n\n' + _am.defaultBazaarFilter.join('\n');
        } },
      ]);
    };

    _am.save_json = function () {
      // Price and note are not saved, they reset on refresh
      const data = { version: _am.equipdata.version };
      setValue('in_equipdata', data);
    };

    _am.check_all = function (checked) {
      _am.equiplist.forEach((eq) => { eq.node.checkbox.checked = checked; });
      if (typeof _window.update_selected_count === 'function') {
        _window.update_selected_count();
      }
    };

    _am.toggle_filter = function () {
      const allRows = $qsa('#equiplist table tr[onmouseover*="hover_equip"]');
      const isFiltering = _am.node.toggleFilter.value === 'Hide Trash';
      const bazaarFilter = (getValue('am_bazaar', _am.defaultBazaarFilter) || []).map(f => f.trim()).filter(f => f);
      ...
      _am.node.toggleFilter.value = isFiltering ? 'Show All' : 'Hide Trash';
    };

    _am.get_base = function (eq) {
      // Isekai uses Base values in title attributes, extract them from tooltip
      // Support both English "Base:" and Chinese "基础:"
      eq.node.lpr.value = '...';

      // Simulate hover to trigger tooltip
      const hoverEvent = new MouseEvent('mouseover', { bubbles: true, cancelable: true });
      eq.node.tr.dispatchEvent(hoverEvent);

      // Wait for tooltip to appear and extract Base values
      setTimeout(() => {
        const baseValues = [];
        // Search for both English and Chinese versions
        const tooltipDivs = document.querySelectorAll('div[title*="Base:"], div[title*="基础:"]');

        tooltipDivs.forEach(div => {
          const title = div.getAttribute('title');
          // Match both "Base: 123" and "基础: 123"
          const match = title.match(/(?:Base|基础):\s*(\d+)/i);
          if (match) {
            const base = match[1];
            const statName = div.querySelector('div:first-child')?.textContent.trim();
            if (statName) {
              baseValues.push(`${statName}:${base}`);
            }
          }
        });

        if (baseValues.length > 0) {
          eq.node.note.value = baseValues.join(' ');
        } else {
          eq.node.note.value = eq.info.soulbound ? 'Soulbound' : eq.info.level ? `Lv.${eq.info.level}` : 'No base data';
        }
        ...
        eq.node.tr.dispatchEvent(mouseoutEvent);
      }, 300);
    };

    // Add styles
    GM_addStyle(/*css*/`
      .hvut-am-controls { padding: 8px 5px; background: #EDECDA; box-sizing: border-box; width: 100%; }
      .hvut-am-side { display: flex; flex-direction: column; gap: 3px; }
      .hvut-am-side input[type="button"] { width: 100%; box-sizing: border-box; }
      #equiplist tr { position: relative; }
      .hvut-am-sub { position: absolute; right: 80px; top: 50%; transform: translateY(-50%); white-space: nowrap; z-index: 1; }
      .hvut-am-sub .hvut-am-price { width: 30px; margin: 0 1px; padding: 1px 2px; font-size: 8pt; border: 1px solid #999; background: #EDECDA; vertical-align: middle; box-sizing: border-box; }
      .hvut-am-sub .hvut-am-note { width: 30px; margin: 0 1px; padding: 1px 2px; font-size: 8pt; border: 1px solid #999; background: #EDECDA; vertical-align: middle; box-sizing: border-box; }
      .hvut-am-sub .hvut-am-lpr { width: 18px; height: 18px; margin: 0 1px; padding: 0; font-size: 8pt; font-weight: bold; cursor: pointer; vertical-align: middle; opacity: 0; transition: opacity 0.2s; box-sizing: border-box; }
      .hvut-am-note:hover + .hvut-am-lpr, .hvut-am-lpr:hover { opacity: 1; }
      #equiplist tr.hvut-eq-Superior { background-color: #ccc; }
      #equiplist tr.hvut-eq-Exquisite { background-color: #ce9; }
      #equiplist tr.hvut-eq-Magnificent { background-color: #bdf; }
      #equiplist tr.hvut-eq-Legendary { background-color: #fd8; }
    ...
    const isOrganizeScreen = !_query.screen || _query.screen === 'organize';

    // Add UI buttons and event listeners (organize page and default landing)
    if (isOrganizeScreen) {
      // Add UI buttons container - insert after last armory tab (Salvage)
      const armoryLeft = $id('armory_left');
      if (armoryLeft) {
        // Find the last <a> tag (Salvage tab) and insert after it
        const lastTab = armoryLeft.querySelector('a:last-of-type');
        if (lastTab) {
          _am.node.controls = $element('div', [lastTab, 'afterend'], ['.hvut-am-controls']);
          _am.node.side = $element('div', _am.node.controls, ['.hvut-am-side']);
          $input(['button', 'Equip Code'], _am.node.side, null, () => { _am.wts(); });
          $input(['button', 'Select All'], _am.node.side, null, () => { _am.check_all(true); });
          $input(['button', 'Deselect All'], _am.node.side, null, () => { _am.check_all(false); });
        }
      }

      // Add click event listener for LPR buttons
      $id('equiplist').addEventListener('click', _am.click);
    } else if (_query.screen === 'purchase') {
      // Add filter edit button for Purchase page
      const armoryLeft = $id('armory_left');
      if (armoryLeft) {
        const lastTab = armoryLeft.querySelector('a:last-of-type');
        if (lastTab) {
          _am.node.controls = $element('div', [lastTab, 'afterend'], ['.hvut-am-controls']);
          _am.node.side = $element('div', _am.node.controls, ['.hvut-am-side']);
          _am.node.toggleFilter = $input(['button', 'Show All'], _am.node.side, null, () => { _am.toggle_filter(); });
          $input(['button', 'Edit Filter'], _am.node.side, null, () => { _am.edit_filter(); });
        }
      }
    }

    // Parse equipment list (coloring works on all pages)
    _am.parse_list();

    // Inject "All" into filterbar and aggregate lists per module
    // Only enable All mode for organize and modify screens
    (function () {
      const filterbar = $id('filterbar');
      if (!filterbar) { return; }

      // Only enable All mode for organize, modify, and sell tabs
      //const enableAllMode = ['organize', 'modify', 'sell'].includes(_query.screen);
     // if (!enableAllMode) { return; }

      // Build All link
      const allHref = `?s=Bazaar&ss=am&screen=${_query.screen}`;
      $element('a', [filterbar, 'afterbegin'], { href: allHref, innerHTML: '<div>All</div>' });
      ...
      if (_query.filter) {
        ...
        filterbar.children[0].children[0].classList.add('cfbs');
        // Original first item becomes inactive
        filterbar.children[1] && filterbar.children[1].children[0].classList.remove('cfbs');
        filterbar.children[1] && filterbar.children[1].children[0].classList.add('cfb');
      }

      const getEqCheckboxes = () => (
        equipform ? Array.from(equipform.elements).filter((el) => el.name === 'eqids[]') : []
      );

      const ensureDynamicSelectHandlers = () => {
        if (_am.override_select_handlers) { return; }
        _am.override_select_handlers = true;

        _window.update_selected_count = function () {
          const checkboxes = getEqCheckboxes();
          selected_soulbound = 0;
          selected_count = 0;
          selected_rarity = [];
          ...
          checkboxes.forEach((el) => {
            ...
            selected_count += 1;
            const eq = get_eqdata(parseInt(el.value, 10));
            if (eq !== undefined) {
              const rarity = eq.q;
              selected_rarity[rarity] = (selected_rarity[rarity] ?? 0) + 1;
              if (eq.s) {
                selected_soulbound += 1;
              }
            }
          });
          ...
          if (multiselect) {
            ...
            if (equipcount) {
              const checkedAttr = selected_count < total ? '' : ' checked="checked"';
              equipcount.innerHTML = `<input type="checkbox" onclick="select_all()"${checkedAttr} /><span></span> Selected ${selected_count} of ${total} matching equipment available to ${title}`;
            }
          }

          update_iteminfo();
          const submitButton = document.getElementById('equipsubmit');
          if (submitButton) {
            submitButton.disabled = !block_submit && selected_count > 0 ? '' : 'disabled';
          }
        };

        _window.select_all = function () {
          last_clicked_eqid = 0;
          const checkboxes = getEqCheckboxes();
          const total = checkboxes.length;
          const shouldCheck = selected_count < total;
          checkboxes.forEach((el) => { el.checked = shouldCheck; });
          _window.update_selected_count();
        };
      };

      // If in All mode (no filter param), aggregate all categories for this screen
      if (!_query.filter) {
        const equipTable = $qs('#equiplist table');
        if (!equipTable) { return; }

        ensureDynamicSelectHandlers();

        // Derive available filters from the filterbar (exclude duplicates: equipped/new/salvaged)
        const exclude = new Set(['equipped', 'new', 'salvaged']);
        const filters = Array.from(filterbar.querySelectorAll('a'))
          .map(a => {
            const m = /[?&]filter=([^&#]+)/.exec(a.getAttribute('href'));
        ...
        const tbody = equipTable.tBodies[0];
        let eqselallRow = null;
        const rowsToRemove = [];
        Array.from(tbody.rows).forEach(tr => {
          if (tr.classList.contains('eqselall')) {
            eqselallRow = tr;
          } else {
            rowsToRemove.push(tr);
          }
        });
        rowsToRemove.forEach(tr => tbody.removeChild(tr));

        const mergeEqitems = (doc) => {
          const scripts = doc.querySelectorAll('script');
          for (const script of scripts) {
            const text = script.textContent || '';
            const match = /const\s+eqitems\s*=\s*(\{[\s\S]*?\})\s*;\s*const\s+itemdata/m.exec(text);
            const itemMatch = /const\s+itemdata\s*=\s*(\{[\s\S]*?\})\s*;/.exec(text);
            if (match) {
              try {
                const parsed = JSON.parse(match[1]);
                if (typeof eqitems !== 'undefined' && eqitems && typeof eqitems === 'object') {
                  Object.assign(eqitems, parsed);
                } else if (_window.eqitems && typeof _window.eqitems === 'object') {
                  Object.assign(_window.eqitems, parsed);
                } else {
                  _window.eqitems = parsed;
                }
              } catch (err) {
                console.warn('[HV Utils] Failed to merge eqitems for Armory all-view:', err);
              }
            }
            if (itemMatch) {
              try {
                const parsedItems = JSON.parse(itemMatch[1]);
                if (typeof itemdata !== 'undefined' && itemdata && typeof itemdata === 'object') {
                  Object.assign(itemdata, parsedItems);
                } else if (_window.itemdata && typeof _window.itemdata === 'object') {
                  Object.assign(_window.itemdata, parsedItems);
                } else {
                  _window.itemdata = parsedItems;
                }
              } catch (err) {
                console.warn('[HV Utils] Failed to merge itemdata for Armory all-view:', err);
              }
            }
            if (match || itemMatch) {
              break;
        ...
        const syncEqselall = (doc) => {
          const srcRow = doc.querySelector('#equiplist .eqselall');
          if (!srcRow) { return; }
          const clone = document.importNode(srcRow, true);
          if (eqselallRow && eqselallRow.parentNode === tbody) {
            tbody.replaceChild(clone, eqselallRow);
          } else {
            tbody.insertBefore(clone, tbody.firstChild || null);
          }
          eqselallRow = clone;
        };

        // Helper to append rows and preceding headers from a fetched document
        const appendRows = (doc) => {
          const all = Array.from(doc.querySelectorAll('#equiplist table tr'));
          let pendingHeader = null;
          all.forEach(tr => {
            const hoverAttr = tr.getAttribute('onmouseover') || '';
            const isEquip = /hover_equip\(\d+\)/.test(hoverAttr);
            if (!isEquip) {
              // Potential header row: non-hover, has visible text
              const text = tr.textContent && tr.textContent.trim();
              pendingHeader = text ? tr : null;
              return;
            }
            const eid = (/hover_equip\((\d+)\)/.exec(hoverAttr) || [,''])[1];
            if (!eid || seen.has(eid)) {
              // Even if duplicate equip, clear pending header to avoid orphan header duplication
              pendingHeader = null;
              return;
            }
            // If there is a pending header for this group and not yet added, append header first
            if (pendingHeader) {
              const sig = (pendingHeader.textContent || '').trim();
              if (sig && !headerSeen.has(sig)) {
                headerSeen.add(sig);
                equipTable.tBodies[0].appendChild(document.importNode(pendingHeader, true));
              }
              pendingHeader = null;
            }
            // Append the equipment row
            seen.add(eid);
            equipTable.tBodies[0].appendChild(document.importNode(tr, true));
          });
        };

        // All mode: directly fetch filter=weapon_1handed as base, then merge others
        (async () => {
          // Simple loading hint
          equipTable.classList.add('hvut-eq-loading');
          try {
            // First, fetch weapon_1handed as the base
            const baseFilter = 'weapon_1handed';
            try {
              const html = await $ajax.fetch(`?s=Bazaar&ss=am&screen=${_query.screen}&filter=${encodeURIComponent(baseFilter)}`);
              const doc = $doc(html);
              syncEqselall(doc);
              mergeEqitems(doc);
              appendRows(doc);
            } catch (e) {
              // ignore base filter error
            }

            // Then fetch other filters (exclude weapon_1handed to avoid duplicate)
            const otherFilters = filters.filter(f => f !== baseFilter);
            for (const f of otherFilters) {
              try {
                const html = await $ajax.fetch(`?s=Bazaar&ss=am&screen=${_query.screen}&filter=${encodeURIComponent(f)}`);
                const doc = $doc(html);
                mergeEqitems(doc);
                appendRows(doc);
              } catch (e) {
                // ignore per-filter errors to keep All usable
              }
            }
            const checkboxes = Array.from(equipTable.querySelectorAll('input[name="eqids[]"]'));
            if (typeof _window.selectable_count !== 'undefined') {
              _window.selectable_count = checkboxes.length;
            }
            if (typeof _window.selected_count !== 'undefined') {
              _window.selected_count = checkboxes.filter((cb) => cb.checked).length;
            }
            if (typeof _window.update_selected_count === 'function') {
              _window.update_selected_count();
            }
            const masterCheckbox = equipTable.querySelector('.eqselall input[type="checkbox"]');
            if (masterCheckbox) {
              masterCheckbox.checked = false;
              masterCheckbox.indeterminate = false;
            }
            // Re-parse to enable coloring and controls
            _am.parse_list();
          } finally {
            equipTable.classList.remove('hvut-eq-loading');
          }
        })();
      }
    })();
...
if (settings.shrine && _query.s === 'Bazaar' && _query.ss === 'ss') {
  ...
  _ss.request = async function (iid, select_reward_type, select_reward_slot) {
    ...
    get_message(doc, true).forEach((msg) => {
      ...
      } else if (msg.includes('Sold it for') || msg.includes('Sold the remains for')) { // Isekai: '(Sold the remains for X credits)'
        _ss.equip.sold++;
      } else if (msg.includes('Salvaged it for') || msg.includes('Salvaged the remains for')) { // Isekai might have similar format
        _ss.equip.salvaged++;
  ...
  $ajax.fetch(!_isekai ? '?s=Character&ss=in' : '?s=Bazaar&ss=am&screen=organize&filter=new').then((html) => {
    if (!_isekai) {
      const exec = />Equip Slots: (\d+)(?: \+ (\d+))? \/ (\d+)</.exec(html);
      if (exec) {
        ...
        _ss.node.results_equip.value = 'Equip Slots: N/A';
      }
    } else {
      // Isekai: parse only Inventory Capacity from Armory
      const inv = /Inventory Capacity:<\/td><td>(\d+)<\/td><td>\/<\/td><td>(\d+)/.exec(html);
      if (inv) {
        const invUsed = parseInt(inv[1]), invCap = parseInt(inv[2]);
        _ss.equip.current = invUsed;
        _ss.equip.capacity = invCap;
        _ss.node.results_equip.value = `Inventory Capacity: ${invUsed} / ${invCap}`;
      } else {
        _ss.node.results_equip.value = 'Inventory Capacity: N/A';
      }
    }
  }, () => {
    _ss.node.results_equip.value = 'Equip Slots: N/A';
...
if (settings.moogleMail && _query.s === 'Bazaar' && _query.ss === 'mm') {
  ...
  if (_query.filter === 'new' && _query.hvut !== 'disabled') {
    ...
    // Isekai: Equipment divs are in #mmail_attachequip, but we need to show them in our UI
    if (_isekai && $id('mmail_attachequip')) {
      const isekaiEquipList = $qs('.equiplist', $id('mmail_attachequip'));
      if (isekaiEquipList) {
        // Reference the original list for parsing, but also attach it to our UI
        _mm.node.equip_list = isekaiEquipList;
        // Make it visible in our container (hide the original location if needed)
        _mm.node.equip_attach.appendChild(isekaiEquipList);
      } else {
        _mm.node.equip_list = $qs('.equiplist') || $element('div', null, ['.equiplist nosel']);
        _mm.node.equip_attach.appendChild(_mm.node.equip_list);
      }
    } else {
      // Persistent: Use original behavior - move the equiplist into our container
      _mm.node.equip_list = $qs('.equiplist') || $element('div', null, ['.equiplist nosel']);
      _mm.node.equip_attach.appendChild(_mm.node.equip_list);
    }
    ...
    // Common function to initialize equipment list UI
    _mm.init_equip_list = function() {
      _mm.equip_list.forEach((eq) => {
        eq.visible = true;
    ...
    // Check if we need to load Isekai equipment data
    const equipDivs = $qsa('div[onmouseover*="equips.set"]', _mm.node.equip_list);
    let needsIsekaiData = false;

    if (_isekai && equipDivs.length > 0) {
      // Check if the first equipment div has corresponding dynjs data
      const firstEid = /equips\.set\((\d+),/.test(equipDivs[0].getAttribute('onmouseover')) && RegExp.$1;
      if (firstEid && !$equip.dynjs_equip[firstEid]) {
        needsIsekaiData = true;
      }
    }

    // In Isekai, we need to load equipment dynjs data from Armory page
    if (needsIsekaiData) {
      // Initialize empty list first to prevent errors
      _mm.equip_list = [];

      // Async load equipment data
      (async function() {
        try {
          const html = await $ajax.fetch('?s=Bazaar&ss=am&screen=organize');
          const doc = $doc(html);

          // Load dynjs_equip data from external script
          const scriptTag = $qs('script[src*="/dynjs/"][src*="/equip-"]', doc);
          if (scriptTag) {
            const dynjs_html = await $ajax.fetch(scriptTag.src + '?t=' + Date.now());
            const dynjs_match = /var dynjs_equip = (\{.*?\});/s.exec(dynjs_html);
            if (dynjs_match) {
              Object.assign($equip.dynjs_equip, JSON.parse(dynjs_match[1]));
            }
          }

          // Parse equipment list with loaded data
          _mm.equip_list = $equip.list(_mm.node.equip_list) || [];
          _mm.init_equip_list();
        } catch (e) {
          console.error('[HVUT MM] Failed to load Isekai equipment data:', e);
          _mm.equip_list = [];
        }
      })();
    } else {
      // Non-Isekai or dynjs_equip already available: use normal flow
      _mm.equip_list = $equip.list(_mm.node.equip_list) || [];
      _mm.init_equip_list();
    }

    if ($id('mmail_attachequip')) {
      const equipElem = $id('equip');
      if (equipElem) {
        equipElem.id += '_';
      }
  ...
  if (settings.equipEnchant) {
    var $battle = {
      ...
      load: async function (eid) {
        const eq = $battle.get(eid);
        // Isekai removed enchant feature; skip fetching and show placeholder
        if (_isekai) {
      ...
      parse: function (eid, doc) {
        ...
        const eqLine = div ? $qs('.eq', div) : null;
        const eqText = eqLine && eqLine.children && eqLine.children[1] ? eqLine.children[1].textContent : '';
        const exec = /Condition: (\d+) \/ (\d+) \((\d+)%\)/.exec(eqText || '');
        const eq = $battle.get(eid);
        if (exec) {
        ...
        const enchant = div ? $qsa('#ee > span', div) : [];
      ...
      repair: async function (eid) {
        ...
        if (_isekai) {
          try {
            const html = await $ajax.fetch('?s=Bazaar&ss=am&screen=repair&filter=equipped');
            const doc = $doc(html);
            await $battle.load_dynjs(doc);
            const postoken = $qs('input[name="postoken"]', doc)?.value;
            const eidList = [];
            const conditionByEid = {};
            $qsa('#equiplist table tr[onmouseover*="hover_equip("]', doc).forEach((tr) => {
              const onOver = tr.getAttribute('onmouseover') || '';
              const m = /hover_equip\((\d+)\)/.exec(onOver);
              if (!m) return;
              const id = m[1];
              const tds = tr.querySelectorAll('td');
              const pctText = (tds[tds.length - 1]?.textContent || '').trim();
              const pct = parseInt(pctText.replace('%', ''), 10);
              conditionByEid[id] = Number.isNaN(pct) ? 100 : pct;
              eidList.push(id);
            });
            let scripts = Array.from(doc.querySelectorAll('script'));
            let eqitemsJSON = null;
            let itemdataJSON = null;
            for (const s of scripts) {
              const txt = s.textContent || '';
              if (!txt) continue;
              if (eqitemsJSON == null) {
                const m1 = txt.match(/const\s+eqitems\s*=\s*(\{[\s\S]*?\});/);
                if (m1) eqitemsJSON = m1[1];
              }
              if (itemdataJSON == null) {
                const m2 = txt.match(/const\s+itemdata\s*=\s*(\{[\s\S]*?\});/);
                if (m2) itemdataJSON = m2[1];
              }
              if (eqitemsJSON && itemdataJSON) break;
            }
            let eqitems = {};
            let itemdata = {};
            try {
              if (eqitemsJSON) eqitems = JSON.parse(eqitemsJSON);
              if (itemdataJSON) itemdata = JSON.parse(itemdataJSON);
            } catch (e) {
              // If JSON.parse fails due to trailing commas/spaces, try a safer eval via Function
              try {
                if (eqitemsJSON) eqitems = (new Function('return ' + eqitemsJSON))();
                if (itemdataJSON) itemdata = (new Function('return ' + itemdataJSON))();
              } catch (_) {}
            }
            const idToName = {};
            Object.keys(itemdata).forEach((id) => { idToName[id] = itemdata[id].n; });
            const repairallAgg = {};
            $battle.equips.forEach((beq) => {
              const ejson = eqitems[String(beq.info.eid)];
              if (ejson && ejson.m) {
                const perEq = {};
                Object.entries(ejson.m).forEach(([iid, cnt]) => {
                  const name = idToName[iid] || iid;
                  perEq[name] = cnt;
                  const pct = conditionByEid[String(beq.info.eid)];
                  if (pct != null && pct < 100) {
                    repairallAgg[name] = (repairallAgg[name] || 0) + cnt;
                  }
                });
                beq.data.repair = perEq;
              } else {
                beq.data.repair = {};
              }
            });
            const hasAny = Object.keys(repairallAgg).length > 0;
            $battle.repair.repairall = hasAny ? repairallAgg : 0;
            if (hasAny) {
              Object.entries(repairallAgg).forEach(([n, c]) => {
                const s = $item.count(n);
            ...
            if (eq) {
              $battle.view(eq.info.eid);
            } else {
              $battle.view($battle.current);
            }
            if (postoken) {
              const shouldRepairAll = (eid === 'all');
              const targetIds = shouldRepairAll
                ? eidList.filter((id) => (conditionByEid[id] ?? 100) < 100)
                : (eq ? [String(eq.info.eid)] : []);
              if (targetIds.length > 0) {
                let body = `postoken=${encodeURIComponent(postoken)}`;
                targetIds.forEach((id) => { body += `&eqids[]=${encodeURIComponent(id)}`; });
                await $ajax.fetch('?s=Bazaar&ss=am&screen=repair&filter=equipped', body);
                await $battle.load_inventory();
                await $battle.repair();
              }
            }
            return;
          } catch (e) {
            console.warn('[HV Utils] Isekai repair parse failed:', e);
          }
        }
        ...
        const repairallElem = $id('repairall', doc);
        if (!repairallElem || !repairallElem.nextElementSibling) {
          console.warn('[HV Utils] Cannot find repairall element (Isekai not supported)');
          return;
        }
        const repairall = /Requires: (.+)/.test(repairallElem.nextElementSibling.textContent) && RegExp.$1;
      ...
      load_dynjs: async function (doc) {
        if (_isekai) {
          if (!Object.keys($equip.dynjs_equip).length) {
            try {
              const html = await $ajax.fetch('?s=Character&ss=eq');
              const eqDoc = $doc(html);
              const scripts = $qsa('script[src*="/dynjs/"][src*="/equip-"]', eqDoc);
              for (const script of scripts) {
                const jsText = await $ajax.fetch(script.src + '?t=' + Date.now());
                const equipMatch = /(?:var|let|const)\s+dynjs_equip\s*=\s*(\{[\s\S]*?\});/s.exec(jsText);
                if (equipMatch) {
              ...
              console.warn('[HV Utils] Isekai equip dynjs fetch failed:', err);
            }
          }
          ...
          $battle.equips.forEach((eq) => {
            ...
            const match = /Condition:\s*(\d+)%/.exec(dynjs.d);
            if (match) {
              eq.info.condition = parseInt(match[1], 10);
              eq.info.durability = null;
              eq.info.cdt = eq.info.condition / 100;
              $battle.display_condition(eq.info.eid);
            }
          });
          return;
        ...
        // Persistent (and fallback): use dynjs from current document
        const scriptTag = $qs('script[src*="/dynjs/"]', doc);
        if (!scriptTag) {
          console.warn('[HV Utils] Cannot find dynjs script tag');
          return;
        }
      ...
      display_condition: function (eid) {
        ...
        if (_isekai) {
          ...
          const pct = typeof eq.info.condition === 'number' ? eq.info.condition : parseInt(eq.info.condition, 10);
          eq.node.cdt.textContent = `${pct}%`;
          eq.node.cdt.className = pct < 20 ? 'hvut-bt-cdt1' : '';
          return;
        }
      ...
      init: function () {
        ...
        if (_isekai) {
          // Hide ENCHANT section entirely in Isekai
          $battle.node.enchant.style.display = 'none';
        }
